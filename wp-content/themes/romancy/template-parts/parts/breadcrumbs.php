...
if( ! ( class_exists( 'woocommerce' ) && is_woocommerce() ) ){
    if( get_post_meta(  romancy_get_current_id() ,'romancy_met_show_breadcrumbs', true ) != 'no' ){
    		...
    		$html = '<div id="breadcrumbs">';
				$html .= '<ul class="breadcrumb">';
			        ...
			        $html .= '<li><a href="' . esc_url( home_url('/') ) . '" title="'.esc_attr__('Home', 'romancy').'">' . esc_attr__('Home', 'romancy') . '</a></li> ';
			        ...
			        if ( is_category() ) {
				        ...
				        $html .= $separator.'<li>'.esc_html__('Archive by date', 'romancy').' '.get_the_time('d').'</li>';

			        } elseif ( is_month() ) {

				        $html .= $separator.'<li><a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a></li>';
				        $html .= $separator.'<li>' . esc_html__('Archive by month', 'romancy').' ' . get_the_time('F') . '</li>';

			        } elseif ( is_year() ) {

			        	$html .= $separator.'<li>' . esc_html__('Archive by year', 'romancy').' ' . get_the_time('Y') . '</li>';
			            ...
			            $html .=  $separator.'<li>' . esc_html__('Search results for', 'romancy').' ' . get_search_query() . '</li>';
			        	...
			        	$html .= $separator.'<li>' . esc_html__('Archive by tag', 'romancy').' ' . single_tag_title('', false) . '</li>';

			        } elseif ( is_author() ) {

			        	global $author;
			        	$userdata = get_userdata($author);
			        	$html .= $separator.'<li>' . esc_html__('Articles posted by', 'romancy').' ' . $userdata->display_name . '</li>';

			        } elseif ( is_home() ){

			        	$html .= $separator.'<li>' . esc_html__('Blog', 'romancy').'&nbsp;' . '</li>';

			        }elseif ( is_404() ) {

			        	$html .= $separator.'<li>' . esc_html__('Page not found', 'romancy') . '</li>';
